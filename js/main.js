   ...
   Util.startLoader();
    $.ajax({
      ...
      success: function(response) {
        ...
        $("#shopList").empty();
        var r = $.parseJSON(response);
        $ul = $("<table width='100%'>").appendTo($('#shopList'));
        $.each(r.results, function(idx, shop) {
          var $li = $('<tr><td><a href="'+shop.store_link+'"><img src="'+shop.store_img+'" class="ad"></a></td><td min-width="50%"><b><u>'+shop.store_name+'</b></u><br>blahblah blah blah blahblahblah blah<br>blah blahblahblah balh balh blah<br>blah</td></tr>');
          $li.appendTo($ul);
