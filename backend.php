<?php
  ...
  if($fn == 'register_new_user') {
    ...
    if(!isset($_GET['num_activity'])) {
      ...
      $user_id = $_SESSION['user_id'];
      try {
        $q = $db->prepare("select * from UserActivity where user_id in(select f.friend_id from isFriends f where f.user_id=:user_id) or user_id=:user_id order by date limit $num_activity;");
        $q->execute(array(':user_id' => $user_id));
        $response->results = $q->fetchAll();
...
//add friend
    ...
    if(isset($_SESSION['user_id'])) {
      try {
        $q = $db->prepare('insert ignore into isFriends values (user_id, friend_id) values (:user_id,:friend_id, now())');
